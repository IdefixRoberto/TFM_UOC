...
class ClientsModel extends Mysql {
    ...
    public function selectClients()
    {
        $sql = "SELECT p.id, p.identificacio, p.nom, p.cognoms, p.telefono, p.email, p.rolid, p.estat, p.status, p.contrasenya, p.sexe, p.toke, r.idrol, r.nombre_rol
                ...
                ON p.rolid = r.idrol 
                WHERE p.status != 0
                AND p.rolid = 6"; // Aquí la corrección es usar "=" en vez de "=="
        $request = $this->select_all($sql);
        return $request;
    }
    

    public function selectClient(int $val)
    {
    ...
    public function deleteClient(int $idUsuario)
    {
